...
#pragma parameter sat "Saturation" 1.25 0.0 2.0 0.05
...
// Method to override if you want to change the border.
vec4 effect_on_border() {
    vec4 border_effect = COMPAT_TEXTURE(Background, TEX1);
    border_effect += vec4(vec3(BRIGHTNESS), border_effect.w);
...
vec2 coord_effect;
if (corner != 0.0) coord_effect = pC4 * corner0(pC4); else coord_effect = pC4;

vec2 frag_coord = (coord_effect * (TextureSize.xy/InputSize.xy));
